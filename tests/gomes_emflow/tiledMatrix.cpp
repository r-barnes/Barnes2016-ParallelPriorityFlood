...
class tiledMatrix {
	public:
		...
		bool isTileInMemory(int i, int j);
		T get(int i, int j);
		...
		tiledMatrix(int nrows,int ncolumns,int tilesWidth,int nTilesMemoria,string arquivoBase,string arquivoMatriz="");

		~tiledMatrix();

		int getNumTilesMemoria();
		int getTilesWidth();
...
template <class T>
int tiledMatrix<T>::getNumTilesMemoria() {
	return numTilesMemoria;
}


template <class T>
int tiledMatrix<T>::getTilesWidth() {
	return tilesWidth;
}
...
template <class T>
bool tiledMatrix<T>::isTileInMemory(int i, int j) {
	return (posTiles[i/tilesWidth][j/tilesWidth] != -1);	
}
...
template <class T>
tiledMatrix<T>::tiledMatrix(int nrows,int ncolumns,int tilesWidth,int nTilesMemoria,string arquivoBase,string arquivoMatriz) {
	...
	tamanhoMaximoBlocoComprimido = LZ4_compressBound( tilesWidth*tilesWidth*sizeof(T) );
	bufferTempCompressao = new char[ max(tamanhoMaximoBlocoComprimido,tilesWidth*tilesWidth*sizeof(T)) ];
...
void tiledMatrix<T>::set(int i,int j, T valor) {
	...
	sujo[i/tilesWidth][j/tilesWidth] = true;
...
/*
		...
		const char STATUS_NAO_INICIALIZADO = 2;
		const char STATUS_INICIALIZADO_VALOR_CONSTANTE = 3;
		int valorSetadoParaTodosBlocos = T;
...
void tiledMatrix<T>::loadTile2(int i,int j) {
   ...
   /* gettimeofday(&end, NULL);
    ...
    gettimeofday(&start, NULL);		*/
		...
		LZ4_uncompress (bufferTempCompressao, reinterpret_cast<char *>(matrizes[id]) , tilesWidth*tilesWidth*sizeof(T) );	
    ...
    cerr << "Tempo ler: " << tempoLer << endl << endl;*/
		...
		for(int i=0;i<tilesWidth;i++)
			for(int j=0;j<tilesWidth;j++)			
				matrizes[id][i*tilesWidth + j]  = valorSetadoParaTodosBlocos;
...
void tiledMatrix<T>::removeTileDaMemoria(int i,int j) {
  ...
  /*  long tempoComprimir, tempoGravar, seconds, useconds;    
    ...
    struct timeval start, end;
    gettimeofday(&start, NULL);*/
		bytesCompressaoBloco[i][j] = LZ4_compress(reinterpret_cast<char *>(matrizes[id]), bufferTempCompressao, tilesWidth*tilesWidth*sizeof(T) );
 /*   gettimeofday(&end, NULL);
